...
- Run `stow [package name]` if the package has no specific dots and `stow --dir=$HOME/dotfiles/[package name] [os specific subpackage]` if there are.
### Changing packages
- Delete any old symlinks using `stow -D [package name]` if the package has no specific dots and `stow -D --dir=$HOME/dotfiles/[package name] [subpackage]` if there are.
## Format
